...
public class GvlCanadaTest {
  ...
  public void test() throws IOException {
    Gvl gvl =
        new Loader()
            .globalVendorList(
                GvlV2Test.class
                    .getClassLoader()
                    .getResourceAsStream("vendorlist/v2/ca/vendor-list.json"));
    ...
    Assertions.assertEquals(
        Collections.emptyList(), gvl.getVendors().get(0).getImpConsPurposes().get());
    ...
    Assertions.assertEquals(
        Collections.singletonList(1), gvl.getVendors().get(0).getSpecialFeatures());
