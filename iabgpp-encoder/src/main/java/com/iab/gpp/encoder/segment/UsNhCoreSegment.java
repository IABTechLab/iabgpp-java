...
public class UsNhCoreSegment extends AbstractLazilyEncodableSegment<EncodableBitStringFields> {
  ...
  public List<String> getFieldNames() {
    return UsNhField.USNH_CORE_SEGMENT_FIELD_NAMES;
  }
  ...
  protected EncodableBitStringFields initializeFields() {
    ...
    fields.put(UsNhField.KNOWN_CHILD_SENSITIVE_DATA_CONSENTS, new EncodableFixedIntegerList(2, Arrays.asList(0, 0, 0))
        .withValidator(nullableBooleanAsTwoBitIntegerListValidator));
  ...
  public void validate() {
    Integer saleOptOutNotice = ((EncodableFixedInteger) fields.get(UsNhField.SALE_OPT_OUT_NOTICE)).getValue();
    Integer saleOptOut = ((EncodableFixedInteger) fields.get(UsNhField.SALE_OPT_OUT)).getValue();
    Integer targetedAdvertisingOptOutNotice =
        ((EncodableFixedInteger) fields.get(UsNhField.TARGETED_ADVERTISING_OPT_OUT_NOTICE)).getValue();
    Integer targetedAdvertisingOptOut =
        ((EncodableFixedInteger) fields.get(UsNhField.TARGETED_ADVERTISING_OPT_OUT)).getValue();
    Integer mspaServiceProviderMode =
        ((EncodableFixedInteger) fields.get(UsNhField.MSPA_SERVICE_PROVIDER_MODE)).getValue();
    Integer mspaOptOutOptionMode =
        ((EncodableFixedInteger) fields.get(UsNhField.MSPA_OPT_OUT_OPTION_MODE)).getValue();

    if (saleOptOutNotice == 0) {
      if (saleOptOut == 1 || saleOptOut == 2) {
        throw new ValidationException(
            "Invalid usnh sale notice / opt out combination: {" + saleOptOutNotice + " / " + saleOptOut + "}");
      }
    ...
    if (targetedAdvertisingOptOutNotice == 0) {
      if (targetedAdvertisingOptOut == 1 || targetedAdvertisingOptOut == 2) {
        throw new ValidationException("Invalid usnh targeted advertising notice / opt out combination: {"
    ...
    if (mspaServiceProviderMode == 0) {
      if (saleOptOutNotice == 1 || saleOptOutNotice == 2) {
        throw new ValidationException("Invalid usnh mspa service provider mode / sale opt out notice combination: {"
      ...
      if (mspaOptOutOptionMode == 0 || mspaOptOutOptionMode == 1) {
        throw new ValidationException("Invalid usnh mspa service provider / opt out option modes combination: {"
