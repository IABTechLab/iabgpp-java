...
public class UsNeCoreSegment extends AbstractLazilyEncodableSegment<EncodableBitStringFields> {
  ...
  public void validate() {
    Integer saleOptOutNotice = ((EncodableFixedInteger) fields.get(UsNeField.SALE_OPT_OUT_NOTICE)).getValue();
    Integer saleOptOut = ((EncodableFixedInteger) fields.get(UsNeField.SALE_OPT_OUT)).getValue();
    Integer targetedAdvertisingOptOutNotice =
        ((EncodableFixedInteger) fields.get(UsNeField.TARGETED_ADVERTISING_OPT_OUT_NOTICE)).getValue();
    Integer targetedAdvertisingOptOut =
        ((EncodableFixedInteger) fields.get(UsNeField.TARGETED_ADVERTISING_OPT_OUT)).getValue();
    Integer mspaServiceProviderMode =
        ((EncodableFixedInteger) fields.get(UsNeField.MSPA_SERVICE_PROVIDER_MODE)).getValue();
    Integer mspaOptOutOptionMode =
        ((EncodableFixedInteger) fields.get(UsNeField.MSPA_OPT_OUT_OPTION_MODE)).getValue();

    if (saleOptOutNotice == 0) {
      if (saleOptOut == 1 || saleOptOut == 2) {
        throw new ValidationException(
            "Invalid usne sale notice / opt out combination: {" + saleOptOutNotice + " / " + saleOptOut + "}");
      }
    ...
    if (targetedAdvertisingOptOutNotice == 0) {
      if (targetedAdvertisingOptOut == 1 || targetedAdvertisingOptOut == 2) {
        throw new ValidationException("Invalid usne targeted advertising notice / opt out combination: {"
    ...
    if (mspaServiceProviderMode == 0) {
      if (saleOptOutNotice == 1 || saleOptOutNotice == 2) {
        throw new ValidationException("Invalid usne mspa service provider mode / sale opt out notice combination: {"
      ...
      if (mspaOptOutOptionMode == 0 || mspaOptOutOptionMode == 1) {
        throw new ValidationException("Invalid usne mspa service provider / opt out option modes combination: {"
