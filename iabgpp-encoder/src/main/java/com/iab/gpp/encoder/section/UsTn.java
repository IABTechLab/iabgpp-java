...
public class UsTn extends EncodableSection<UsTnField> {
  ...
  public UsTn() {
    super(new UsTnCoreSegment(), new UsTnGpcSegment());
  }
  ...
  protected void doDecode(CharSequence encodedString) {
    ...
    if(encodedSegments.size() > 1) {
      getSegment(1).setFieldValue(UsTnField.GPC_SEGMENT_INCLUDED, true);
      getSegment(1).decode(encodedSegments.get(1));
    } else {
      getSegment(1).setFieldValue(UsTnField.GPC_SEGMENT_INCLUDED, false);
    }
  }
  ...
  protected CharSequence doEncode() {
    ...
    if(size() >= 2 && getSegment(1).getFieldValue(UsTnField.GPC_SEGMENT_INCLUDED).equals(true)) {
