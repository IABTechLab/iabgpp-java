...
public class UsCoV1CoreSegment extends AbstractLazilyEncodableSegment<EncodableBitStringFields> {
  ...
  protected EncodableBitStringFields initializeFields() {
    ...
    fields.put(UsCoV1Field.SHARING_NOTICE, new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.SALE_OPT_OUT_NOTICE, new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.TARGETED_ADVERTISING_OPT_OUT_NOTICE,
        new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.SALE_OPT_OUT, new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.TARGETED_ADVERTISING_OPT_OUT, new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.SENSITIVE_DATA_PROCESSING,
        new EncodableFixedIntegerList(2, Arrays.asList(0, 0, 0, 0, 0, 0, 0), v -> {
          for(Integer i : v) {
            if(i < 0 || i > 2) {
              return false;
            }
    ...
    fields.put(UsCoV1Field.KNOWN_CHILD_SENSITIVE_DATA_CONSENTS,
        new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.MSPA_COVERED_TRANSACTION, new EncodableFixedInteger(2, 1, (v -> v >= 1 && v <= 2)));
    fields.put(UsCoV1Field.MSPA_OPT_OUT_OPTION_MODE, new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    fields.put(UsCoV1Field.MSPA_SERVICE_PROVIDER_MODE, new EncodableFixedInteger(2, 0, (v -> v >= 0 && v <= 2)));
    return fields;
  }
  ...
  public void validate() {
    ...
    if (saleOptOutNotice == 0) {
      if (saleOptOut != 0) {
        throw new ValidationException("Invalid usco sale notice / opt out combination: {"
