...
public class UsDeCoreSegment extends AbstractLazilyEncodableSegment<EncodableBitStringFields> {
  ...
  public UsDeCoreSegment(String encodedString) {
    super();
  ...
  public List<String> getFieldNames() {
    return UsDeField.USDE_CORE_SEGMENT_FIELD_NAMES;
  }
  ...
  protected void decodeSegment(String encodedString, EncodableBitStringFields fields) {
    ...
    try {
      ...
      throw new DecodingException("Unable to decode UsDeCoreSegment '" + encodedString + "'", e);
    }
  }

  @Override
  public void validate() {
    Integer saleOptOutNotice = ((EncodableFixedInteger) fields.get(UsDeField.SALE_OPT_OUT_NOTICE)).getValue();
    Integer saleOptOut = ((EncodableFixedInteger) fields.get(UsDeField.SALE_OPT_OUT)).getValue();
    Integer targetedAdvertisingOptOutNotice =
        ((EncodableFixedInteger) fields.get(UsDeField.TARGETED_ADVERTISING_OPT_OUT_NOTICE)).getValue();
    Integer targetedAdvertisingOptOut =
        ((EncodableFixedInteger) fields.get(UsDeField.TARGETED_ADVERTISING_OPT_OUT)).getValue();
    Integer mspaServiceProviderMode =
        ((EncodableFixedInteger) fields.get(UsDeField.MSPA_SERVICE_PROVIDER_MODE)).getValue();
    Integer mspaOptOutOptionMode =
        ((EncodableFixedInteger) fields.get(UsDeField.MSPA_OPT_OUT_OPTION_MODE)).getValue();

    if (saleOptOutNotice == 0) {
      if (saleOptOut == 1 || saleOptOut == 2) {
        throw new ValidationException(
            "Invalid usde sale notice / opt out combination: {" + saleOptOutNotice + " / " + saleOptOut + "}");
      }
    ...
    if (targetedAdvertisingOptOutNotice == 0) {
      if (targetedAdvertisingOptOut == 1 || targetedAdvertisingOptOut == 2) {
        throw new ValidationException("Invalid usde targeted advertising notice / opt out combination: {"
    ...
    if (mspaServiceProviderMode == 0) {
      if (saleOptOutNotice == 1 || saleOptOutNotice == 2) {
        throw new ValidationException("Invalid usde mspa service provider mode / sale opt out notice combination: {"
      ...
      if (mspaOptOutOptionMode == 0 || mspaOptOutOptionMode == 1) {
        throw new ValidationException("Invalid usde mspa service provider / opt out option modes combination: {"
